...
( function($){
	...
	$(function(){
		...
		if( $('.woocommerce-error').length > 0 ){
			$('.woocommerce-error').append('<br/><button id="warning-close" type="submit" class="button">OK</button>');
		...
		if( WooCheckout ){
			var pInfo = $('.woocommerce-checkout').find('p.info'), pWinfo = $('.woocommerce-checkout').find('p.woocommerce-info'),
				wLogin = $('form.login'),
